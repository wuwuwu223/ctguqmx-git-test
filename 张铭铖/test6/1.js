...
$(function(){
	...
	var timer;//定义定时函数
	$('.lunbo .img img').eq(0).show().siblings().hide();//定义默认的显示图片，也就是索引为0的那张图片
	start();//开始实现图片轮播，用到了定时器
	$('b').mouseover(function(){//当鼠标运动到某个小圆点是，切换图片
	...
	function start(){//轮播开始函数
		timer = setInterval(function(){//自动轮播定时函数
			...
			change();//继续执行图片轮播
		},1000)//2000是多久切换一次图片，表示两秒
	};
	function change(){//图片显示函数，这里的fadeOut和fadeIn是图片显示方式是淡入淡出
		$('.lunbo .img img').eq(i).fadeIn(300).siblings().stop().fadeOut(300);//eq选择当前图片，siblings表示排除其他图片，stop表示其他图片停止切换，只切换当前图片
